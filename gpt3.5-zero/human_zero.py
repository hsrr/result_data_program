...
class SolubilityClassifier(object):
    ...
    def __init__(self, train_data: pd.DataFrame, num_samples_for_each_class=2, num_classes=None,
        ...
        self.general_template = """You are a biology expert who has been deeply involved in the field of protein research for years. Given the amino acid sequence of a protein, your task is to predict the property of proteins using your experienced biological Property Prediction knowledge.
        """
        ...
        amino acid sequences interact (`yes`) or not (`no`). Please answer with only `yes` or `no`.Do not make no response.
        """
...
Interaction: 
        ...
        #k=num_samples_for_each_class * num_classes
        if sampling_method == "similar":
            ...
            example_selector = ProteinSimilarityExampleSelector(
                examples=examples, data_dir=selector_cache_dir, k=100,
            )
...
#                                    max_retries=2,
#                                    max_tokens=10)
#         self.llm = ChatOpenAI(model="gpt-4",openai_api_key="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",max_tokens=10)
...
#     print(f"{all_df['target'].value_counts()}")
    ...
    clf = SolubilityClassifier(train_data=dfs["train"], num_samples_for_each_class=args.num_samples_for_each_class,
                               num_classes=None, verbose=False,
                               sampling_method=args.sampling_method, selector_cache_dir=data_dir,
                               id2label_func=lambda x: "yes" if x == 1 else "no")

    num_correct = 0
    num_total = 0
    num_unknown = 0
#     test_0_df = test_df[test_df["target"] == 0].iloc[:100]
#     test_1_df = test_df[test_df["target"] == 1].iloc[:100]
#     test_sample_df = pd.concat([test_0_df, test_1_df], axis=0)
    # test_sample_df = test_sample_df.sample(frac=1.)
    for sample_idx, sample in test_df[::].iterrows():
        sequence, target = sample["sequence"], sample["target"]
        try:
            can_sequence = canonicalize(sequence[0])
        except KeyError as s:
            print(f"idx:{sample_idx},sequence[0],{sequence[0]}，{s}")
            worksheet.cell(row=row, column=1, value=f"idx:{sample_idx},sequence[0],{sequence[0]}，{s}")
            row+=1
            #time.sleep(0.1)
            print("erro")
            ...
            continue
        try:
            can_sequence = canonicalize(sequence[1])
        except KeyError as s:
            print(f"idx:{sample_idx},sequence[1],{sequence[1]}，{s}")
            worksheet.cell(row=row, column=1, value=f"idx:{sample_idx},sequence[1],{sequence[1]}，{s}")
            row+=1
            #time.sleep(0.1)
            print("erro")
            worksheet.cell(row=row, column=1, value="erro")
        ...
        result = clf(protein_1=sequence[0], protein_2=sequence[1])
        label = result["label"]
